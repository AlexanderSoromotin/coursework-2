...
<div class="main-container container">
    <h1 class="page-title">
        Сотрудники
        <button id="addEmployee" class="btn btn-primary" >Добавить</button>
{{--        data-toggle="modal" data-target="#addEmployeeModal"--}}

        <div class="search">
            <input type="text" placeholder="Найти">
            <img class="svg" src="{{ url("img/icons/search.svg") }}" alt="">
        </div>
    </h1>
            ...
            <div class="modal-body">
                ...
                <form>
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="employeeLastname" placeholder="Введите фамилию сотрудника">
                    </div>

                    <div class="form-group">
                        <label for="employeeFirstname">Имя</label>
                        <input type="text" class="form-control" id="employeeFirstname" placeholder="Введите имя сотрудника">
                    </div>

                    <div class="form-group">
                        <label for="employeePatronymic">Отчество</label>
                        <input type="text" class="form-control" id="employeePatronymic" placeholder="Введите отчество сотрудника">
                    </div>
                    <div class="form-group">
                        <label for="employeePosition">Должность</label>
                        <input type="text" class="form-control" id="employeePosition" placeholder="Введите должность сотрудника">
                    </div>
                    <!-- Добавьте другие поля для ввода данных сотрудника, если необходимо -->
...
<script>
    ...
    function updateEmployees () {
        $.ajax({
            ...
            success: (response) => {
                console.log("success GET /employees", response);
                $(".list-items a").remove();
                    ...
                    $(".list-items").append(`
                        <a href="{{ url("/employees") }}/${item.id}">
                            <div class="item">
                                <div class="user-data">
                                    <div class="image">
                                        <img src="{{ url("img/avatars") }}/${item.avatar}" alt="">
                                    </div>
                                    <div class="name">${item.last_name} ${item.first_name} ${item.patronymic}</div>
                                </div>
                                <div class="arrow">
                                    <img src="{{ url("/img/icons/arrow-narrow-right.svg") }}" alt="">
                                </div>
                            </div>
                        </a>
                    `)
    ...
    $("#addEmployee").click(function () {
        $.ajax({
            ...
            success: (response) => {
                console.log("success POST /employees", response);
                updateEmployees();
            },
            error: (response) => {
    ...
    $(".search input").on("input keyup", function () {
        ...
        $(".list-items .item").each(function (i) {
            item = $(".list-items .item:eq(" + i + ")");
